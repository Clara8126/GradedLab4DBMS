...
INSERT INTO PRODUCT VALUES (3 , "ROG LAPTOP"," Windows 10 with 15inch screeni7 processor, 1TB SSD ", "4");
...
/* To display  total number of customers based on gender who have placed orders of worth at least Rs.3000*/
...
/* To display all the orders along with the product name ordered by customer having customer_id = 2 */
...
`order`.pricing_id = supplier_pricing.PRICING_ID and 
 ...
 /* To Display  least expensive product from each category and print the table 
 ...
 /* To Display the Id and Name of the Product ordered after “2021-10-05”*/
 select PRO_ID , PRO_NAME from product where pro_id in 
 (select pro_id from supplier_pricing where PRICING_ID in 
 (select PRICING_ID from e_com.order where ord_data > '2021-10-05'));
 ...
 on product.pro_id = supplier_pricing .prod_id where `order`.ord_date> "2021-10-05";

 
/* to display customer name and gender whose names start or end with characher 'A'*/
...
CASE
WHEN report.Average = 5 THEN 'Excellect Service'
WHEN report.Average = 5 THEN 'Good Service'
WHEN report.Average = 5 THEN 'Average Service'
ELSE 'Poor Service'
